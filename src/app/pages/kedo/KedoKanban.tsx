...
import { DocumentKanbanBoard } from "../../components/kedo/DocumentKanbanBoard"
import { documents } from "../../data/kedo"
import { FileText, Clock, AlertTriangle, CheckCircle } from "lucide-react"

export function KedoKanban() {
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-500 max-w-[1200px] mx-auto h-[calc(100vh-6rem)]">
      {/* Header section */}
      ...
      {/* Navigation Tabs */}
      <div className="flex items-center gap-8 border-b border-slate-200 mt-2 mb-2 shrink-0">
      ...
      <div className="flex flex-col gap-5 min-h-0 flex-1">
        ...
        {/* Toolbar */}
        <div className="flex items-center justify-between bg-white p-4 rounded-2xl shadow-[0px_2px_12px_rgba(0,0,0,0.03)] border border-slate-100/60 shrink-0">
          ...
          <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-lg">
            <Link to="/kedo" className="p-1.5 rounded-md text-slate-500 hover:text-slate-700 hover:bg-slate-200/50 transition-colors" title="Список">
              <List className="w-4 h-4" />
            </Link>
            <Link to="/kedo/kanban" className="p-1.5 rounded-md bg-white shadow-sm text-slate-700" title="Канбан">
        ...
        <div className="flex-1 min-h-0 overflow-x-auto hide-scrollbar pb-4 -mx-1 px-1">
          <DocumentKanbanBoard documents={documents} />
        </div>
